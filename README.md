...
- **Stock Service (Port 3003)**: Manages inventory with optimistic locking to prevent over-selling. Features a 1.5s simulated processing delay (optional) for visibility.
...
### 🍱 Student Portal
- **Real-time Tracking**: Watch your order transition from `Confirmed` → `Stock Verified` → `In Kitchen` → `Ready`.
