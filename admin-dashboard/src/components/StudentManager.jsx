  ...
  const fetchData = async () => {
    try {
      const [studentResp, statsResp] = await Promise.all([
        axios.get(
          `${import.meta.env.VITE_IDENTITY_PROVIDER_URL || "http://localhost:3001"}/students`,
        ),
        axios.get(
          `${import.meta.env.VITE_API_GATEWAY_URL || "http://localhost:3002"}/admin/student-stats`,
        ),
      ]);
      ...
      <div className="pagination">
        ...
        {pages.map((p, i) =>
          p === "..." ? (
            <span
              key={`sep-${i}`}
              style={{ color: "var(--text-muted)", padding: "0 0.5rem" }}
            >
              ...
            </span>
          ) : (
          ...
          <div className="modal-content">
            <h2
              style={{
                marginBottom: "1rem",
                color:
                  modal.type === "error" ? "var(--danger)" : "var(--success)",
              }}
            >
              {modal.title}
            </h2>
          ...
          <tbody>
            {currentStudents.map((student) => (
              <tr key={student.studentId}>
                ...
                <td>
                  <span
                    className={`status-pill ${(stats[student.studentId] || 0) > 0 ? "success" : "warning"}`}
                  >
                    {(stats[student.studentId] || 0) > 0
                      ? "Active"
                      : "Inactive"}
                  </span>
                </td>
